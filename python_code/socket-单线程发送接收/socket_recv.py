...
def main():
    ...
    local_addr = ('', 4415) #  ip地址和端口号，ip一般不用写，表示本机的任何一个ip
    s.bind(local_addr)

     #5.接受数据recvfrom(规定接受数据的大小),接受的端口号就是发出去的端口号，如果不中断这个程>序，那么端口号是不变的
